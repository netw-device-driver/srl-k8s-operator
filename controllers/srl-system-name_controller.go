...
var SystemNameInternalResourceleafRef = map[string]*ElementWithLeafRef{}

var SystemNameExternalResourceleafRef = map[string]*ElementWithLeafRef{}
...
func (r *SrlSystemNameReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlSystemNameReconciler) ValidateExternalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemName, cm *string) (err error) {
	...
	for localLeafRef, leafRefInfo := range SystemNameExternalResourceleafRef {
	...
	r.Log.WithValues("SystemNameExternalResourceleafRef", SystemNameExternalResourceleafRef).Info("External leafref STATUS All")
...
func (r *SrlSystemNameReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemName) (err error) {
	...
	for localLeafRef, leafRefInfo := range SystemNameInternalResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
	...
	r.Log.WithValues("SystemNameInternalResourceleafRef", SystemNameInternalResourceleafRef).Info("leafref STATUS All")
	return nil
}
...
func (r *SrlSystemNameReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !o.DeletionTimestamp.IsZero() && SrlSystemNamehasFinalizer(o) {
		if SrlSystemNamehasOtherFinalizer(o) {
	...
	if o.DeletionTimestamp.IsZero() && SrlSystemNamehasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlSystemName"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
...
func SrlSystemNamelogResult(info *SrlSystemNameReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlSystemName")
		//info.log.Info("stopping reconcile SrlSystemName",
...
func (r *SrlSystemNameReconciler) getConfigMap(ctx context.Context, targetName *string) (*string, error) {
...
/*
	func (r *SrlSystemNameReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlSystemName"] = string(d)
		cm.Data[fmt.Sprintf("SrlSystemName.%s",*oName)] = string(d)
...
/*
	func (r *SrlSystemNameReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlSystemName.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlSystemName.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlSystemNameReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
func (r *SrlSystemNameReconciler) findPathInTree(x1 interface{}, ekvl []ElementKeyValue, idx int) bool {
...
func (r *SrlSystemNameReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlSystemNameReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlSystemNameReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
// SSrlSystemNamehasOtherFinalizer checks if object has other finalizers
func SrlSystemNamehasOtherFinalizer(o *srlinuxv1alpha1.SrlSystemName) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlSystemNameFinalizer {
			return true
		}
	}
	return false
...
func (o *SrlSystemNameStateMachine) handleConfigureSuccess(info *SrlSystemNameReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNameStateMachine) handleConfigureFailed(info *SrlSystemNameReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNameStateMachine) handleDeleteSuccess(info *SrlSystemNameReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlSystemNameStateMachine) handleDeleteFailed(info *SrlSystemNameReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
