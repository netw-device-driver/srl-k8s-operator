...
func (r *SrlSystemNetworkinstanceProtocolsEvpnReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !o.DeletionTimestamp.IsZero() && SrlSystemNetworkinstanceProtocolsEvpnhasFinalizer(o) {
		if SrlSystemNetworkinstanceProtocolsEvpnhasOtherFinalizer(o) {
	...
	if o.DeletionTimestamp.IsZero() && SrlSystemNetworkinstanceProtocolsEvpnhasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlSystemNetworkinstanceProtocolsEvpn"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
...
func SrlSystemNetworkinstanceProtocolsEvpnlogResult(info *SrlSystemNetworkinstanceProtocolsEvpnReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlSystemNetworkinstanceProtocolsEvpn")
		//info.log.Info("stopping reconcile SrlSystemNetworkinstanceProtocolsEvpn",
...
/*
	func (r *SrlSystemNetworkinstanceProtocolsEvpnReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlSystemNetworkinstanceProtocolsEvpn"] = string(d)
		cm.Data[fmt.Sprintf("SrlSystemNetworkinstanceProtocolsEvpn.%s",*oName)] = string(d)
...
/*
	func (r *SrlSystemNetworkinstanceProtocolsEvpnReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlSystemNetworkinstanceProtocolsEvpn.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlSystemNetworkinstanceProtocolsEvpn.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlSystemNetworkinstanceProtocolsEvpnReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
// SSrlSystemNetworkinstanceProtocolsEvpnhasOtherFinalizer checks if object has other finalizers
func SrlSystemNetworkinstanceProtocolsEvpnhasOtherFinalizer(o *srlinuxv1alpha1.SrlSystemNetworkinstanceProtocolsEvpn) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlSystemNetworkinstanceProtocolsEvpnFinalizer {
			return true
		}
	}
	return false
...
func (o *SrlSystemNetworkinstanceProtocolsEvpnStateMachine) handleConfigureSuccess(info *SrlSystemNetworkinstanceProtocolsEvpnReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNetworkinstanceProtocolsEvpnStateMachine) handleConfigureFailed(info *SrlSystemNetworkinstanceProtocolsEvpnReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNetworkinstanceProtocolsEvpnStateMachine) handleDeleteSuccess(info *SrlSystemNetworkinstanceProtocolsEvpnReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlSystemNetworkinstanceProtocolsEvpnStateMachine) handleDeleteFailed(info *SrlSystemNetworkinstanceProtocolsEvpnReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
