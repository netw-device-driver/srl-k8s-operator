...
/*
	func (r *SrlNetworkinstanceNexthopgroupsReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.NetworkinstanceNexthopgroups) (err error) {
		// marshal data to json
		dd := struct {
			NextHopGroups *srlinuxv1alpha1.NetworkinstanceNexthopgroups `json:"next-hop-groups"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range NetworkinstanceNexthopgroupsIntraResourceleafRef {
...
func (r *SrlNetworkinstanceNexthopgroupsReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlNetworkinstanceNexthopgroupshasFinalizer(o) {
		...
		/*
			...
			for s, elementWithLeafRef := range NetworkinstanceNexthopgroupsIntraResourceleafRef {
...
/*
	func (r *SrlNetworkinstanceNexthopgroupsReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstanceNexthopgroups) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range NetworkinstanceNexthopgroupsInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
