...
/*
	func (r *SrlRoutingpolicyPrefixsetReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.RoutingpolicyPrefixset) (err error) {
		// marshal data to json
		dd := struct {
			PrefixSet *srlinuxv1alpha1.RoutingpolicyPrefixset `json:"prefix-set"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range RoutingpolicyPrefixsetIntraResourceleafRef {
...
func (r *SrlRoutingpolicyPrefixsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlRoutingpolicyPrefixsethasFinalizer(o) {
		...
		/*
			validationSuccess := true
			for _, vo := range *o.Spec.SrlRoutingpolicyPrefixset {
...
/*
	func (r *SrlRoutingpolicyPrefixsetReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlRoutingpolicyPrefixset) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range RoutingpolicyPrefixsetInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
