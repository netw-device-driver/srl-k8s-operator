...
var BfdInternalResourceleafRef = map[string]*ElementWithLeafRef{}

var BfdExternalResourceleafRef = map[string]*ElementWithLeafRef{
...
// SrlBfdReconciler reconciles a SrlBfd object
type SrlBfdReconciler struct {
	client.Client
	Log    logr.Logger
...
type SrlBfdTargetReconcileInfo struct {
	Target map[string]*SrlBfdReconcileInfo
}

type SrlBfdReconcileInfo struct {
	target              *string
	resource            *string
...
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlbfds,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlbfds/status,verbs=get;update;patch
...
/*
	func (r *SrlBfdReconciler) updateEventHandler(e event.UpdateEvent) bool {
...
func (r *SrlBfdReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, option controller.Options) error {
	b := ctrl.NewControllerManagedBy(mgr).
		For(&srlinuxv1alpha1.SrlBfd{}).
...
// request for reconciliation of SrlBfd.
func (r *SrlBfdReconciler) NetworkNodeMapFunc(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
...
func (r *SrlBfdReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlBfdReconciler) ValidateExternalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlBfd, cm *string) (err error) {
	r.Log.Info("Validate External LeafRef Dependencies ...")

	// marshal data to json
	dd := struct {
		Bfd *srlinuxv1alpha1.Bfd `json:"bfd"`
	}{
		Bfd: o.Spec.SrlBfd,
	}
	d := make([][]byte, 0)
	...
	for localLeafRef, leafRefInfo := range BfdExternalResourceleafRef {
	...
	r.Log.WithValues("BfdExternalResourceleafRef", BfdExternalResourceleafRef).Info("External leafref STATUS All")

	return nil
}

func (r *SrlBfdReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlBfd) (err error) {
	...
	for localLeafRef, leafRefInfo := range BfdInternalResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
	...
	r.Log.WithValues("BfdInternalResourceleafRef", BfdInternalResourceleafRef).Info("leafref STATUS All")
	return nil
}

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *SrlBfdReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("SrlBfd", req.NamespacedName)

	r.Log.WithValues("ObjectName", req.NamespacedName).Info("reconciling SrlBfd")
	...
	if err := r.Client.Get(ctx, req.NamespacedName, o); err != nil {
		...
		r.Log.WithValues(req.Name, req.Namespace).Error(err, "Failed to get SrlBfd config")
	...
	if o.DeletionTimestamp.IsZero() && !SrlBfdhasFinalizer(o) {
		r.Log.Info(
			"adding finalizer",
			"existingFinalizers", o.Finalizers,
			"newValue", srlinuxv1alpha1.SrlBfdFinalizer,
		)
		o.Finalizers = append(o.Finalizers,
			srlinuxv1alpha1.SrlBfdFinalizer)
		err := r.Update(ctx, o)
		if err != nil {
	...
	if o.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(o) {
		...
		if err := r.saveSrlBfdStatus(ctx, o); err != nil {
	...
	if err != nil {
		...
		case *TargetNotFoundError:
			...
			if !o.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(o) {
				...
				o.Finalizers = removeString(o.Finalizers, srlinuxv1alpha1.SrlBfdFinalizer)
	...
	level := int32(1)
	resource := "srlinux.henderiw.be" + "." + "SrlBfd" + "." + strcase.UpperCamelCase(o.Name)

	dependencies := make([]string, 0)

	deletepaths := make([]string, 0)
	deletepaths = append(deletepaths, "/bfd")
	...
	if o.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlBfd"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
	...
	info := make(map[string]*SrlBfdReconcileInfo)
	...
	for _, target := range t {
		...
		r.Log.Info("configuration status in reconcile",
			"target", target.TargetName,
			"status", initialState)
		info[target.TargetName] = &SrlBfdReconcileInfo{
			ctx:                 ctx,
		...
		stateMachine := newSrlBfdStateMachine(o, r, &target.TargetName, info[target.TargetName])
	...
	for _, target := range t {
		...
		SrlBfdlogResult(info[target.TargetName], result[target.TargetName])
...
func SrlBfdlogResult(info *SrlBfdReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlBfd")
		//info.log.Info("stopping reconcile SrlBfd",
		//	"message", info.o.Status)
	}
}

func (r *SrlBfdReconciler) saveSrlBfdStatus(ctx context.Context, o *srlinuxv1alpha1.SrlBfd) error {
	t := metav1.Now()
	o.Status.DeepCopy()
	...
	if err := r.Client.Status().Update(ctx, o); err != nil {
		r.Log.WithValues(o.Name, o.Namespace).Error(err, "Failed to update SrlBfd ")
		return err
	}
	return nil
}

func (r *SrlBfdReconciler) getConfigMap(ctx context.Context, targetName *string) (*string, error) {
...
/*
	func (r *SrlBfdReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlBfd"] = string(d)
		cm.Data[fmt.Sprintf("SrlBfd.%s",*oName)] = string(d)
...
/*
	func (r *SrlBfdReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlBfd.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlBfd.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlBfdReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
func (r *SrlBfdReconciler) findPathInTree(x1 interface{}, ekvl []ElementKeyValue, idx int) bool {
...
func (r *SrlBfdReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlBfdReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlBfdReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
func (r *SrlBfdReconciler) FindSpecDiff(ctx context.Context, o *srlinuxv1alpha1.SrlBfd) (bool, *[]string, error) {
...
func (r *SrlBfdReconciler) FindTarget(ctx context.Context, o *srlinuxv1alpha1.SrlBfd) ([]*Target, bool, error) {
...
// SrlBfdhasFinalizer checks if object has finalizer
func SrlBfdhasFinalizer(o *srlinuxv1alpha1.SrlBfd) bool {
	return StringInList(o.Finalizers, srlinuxv1alpha1.SrlBfdFinalizer)
}

// SSrlBfdhasOtherFinalizer checks if object has other finalizers
func SrlBfdhasOtherFinalizer(o *srlinuxv1alpha1.SrlBfd) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlBfdFinalizer {
			return true
		}
	}
	return false
...
func (info *SrlBfdReconcileInfo) DeleteCache() error {
	if !info.o.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(info.o) {
...
// Update Cache
func (info *SrlBfdReconcileInfo) UpdateCache(path string) error {

	// marshal data to json
	dd := struct {
		Bfd *srlinuxv1alpha1.Bfd `json:"bfd"`
	}{
		Bfd: info.o.Spec.SrlBfd,
	}
	d := make([][]byte, 0)
	dj, err := json.Marshal(dd)
	if err != nil {
...
type SrlBfdStateMachine struct {
	Object     *srlinuxv1alpha1.SrlBfd
	Reconciler *SrlBfdReconciler
	Target     *string
	TargetName *string
...
// appendEvent
func (info *SrlBfdReconcileInfo) appendEvent(reason, message string) {
	info.events = append(info.events, info.o.NewEvent(reason, message))
}

func newSrlBfdStateMachine(o *srlinuxv1alpha1.SrlBfd,
	reconciler *SrlBfdReconciler, n *string,
	info *SrlBfdReconcileInfo) *SrlBfdStateMachine {
	currentState := o.Status.Target[*n].ConfigStatus
	r := SrlBfdStateMachine{
		Object:     o,
...
type SrlBfdstateHandler func(*SrlBfdReconcileInfo) actionResult

func (o *SrlBfdStateMachine) handlers() map[srlinuxv1alpha1.ConfigStatus]SrlBfdstateHandler {
	return map[srlinuxv1alpha1.ConfigStatus]SrlBfdstateHandler{
...
func (o *SrlBfdStateMachine) updateSrlBfdStateFrom(initialState *srlinuxv1alpha1.ConfigStatus,
	info *SrlBfdReconcileInfo) {
...
func (o *SrlBfdStateMachine) ReconcileState(info *SrlBfdReconcileInfo) actionResult {
	initialState := o.Object.Status.Target[*o.TargetName].ConfigStatus
	defer o.updateSrlBfdStateFrom(initialState, info)

	if o.checkInitiateDelete() {
		// initiate cache delete
		info.log.Info("Initiating SrlBfdStateMachine deletion")
		info.DeleteCache()
	}
...
func (o *SrlBfdStateMachine) checkInitiateDelete() bool {
	if !o.Object.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(o.Object) {
		// Delete requested
		switch *o.NextState {
		default:
			// new state deleting
...
func (o *SrlBfdStateMachine) handleNone(info *SrlBfdReconcileInfo) actionResult {
...
func (o *SrlBfdStateMachine) handleConfiguring(info *SrlBfdReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlBfdStateMachine) handleConfigureSuccess(info *SrlBfdReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlBfdStateMachine) handleConfigureFailed(info *SrlBfdReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlBfdStateMachine) handleDeleting(info *SrlBfdReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlBfdStateMachine) handleDeleteSuccess(info *SrlBfdReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlBfdStateMachine) handleDeleteFailed(info *SrlBfdReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
