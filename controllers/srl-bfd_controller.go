...
/*
	func (r *SrlBfdReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.Bfd) (err error) {
		// marshal data to json
		dd := struct {
			Bfd *srlinuxv1alpha1.Bfd `json:"bfd"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range BfdIntraResourceleafRef {
...
func (r *SrlBfdReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlBfdhasFinalizer(o) {
		...
		/*
			...
			for s, elementWithLeafRef := range BfdIntraResourceleafRef {
...
/*
	func (r *SrlBfdReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlBfd) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range BfdInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
