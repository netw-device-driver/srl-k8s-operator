...
/*
	func (r *SrlNetworkinstanceAggregateroutesReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.NetworkinstanceAggregateroutes) (err error) {
		// marshal data to json
		dd := struct {
			AggregateRoutes *srlinuxv1alpha1.NetworkinstanceAggregateroutes `json:"aggregate-routes"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range NetworkinstanceAggregateroutesIntraResourceleafRef {
...
func (r *SrlNetworkinstanceAggregateroutesReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlNetworkinstanceAggregaterouteshasFinalizer(o) {
		...
		/*
			...
			for s, elementWithLeafRef := range NetworkinstanceAggregateroutesIntraResourceleafRef {
...
/*
	func (r *SrlNetworkinstanceAggregateroutesReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstanceAggregateroutes) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range NetworkinstanceAggregateroutesInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
