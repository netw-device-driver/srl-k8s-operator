...
/*
	func (r *SrlRoutingpolicyAspathsetReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.RoutingpolicyAspathset) (err error) {
		// marshal data to json
		dd := struct {
			AsPathSet *srlinuxv1alpha1.RoutingpolicyAspathset `json:"as-path-set"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range RoutingpolicyAspathsetIntraResourceleafRef {
...
func (r *SrlRoutingpolicyAspathsetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlRoutingpolicyAspathsethasFinalizer(o) {
		...
		/*
			validationSuccess := true
			for _, vo := range *o.Spec.SrlRoutingpolicyAspathset {
...
/*
	func (r *SrlRoutingpolicyAspathsetReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlRoutingpolicyAspathset) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range RoutingpolicyAspathsetInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
