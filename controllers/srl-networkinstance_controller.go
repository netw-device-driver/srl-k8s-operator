...
var NetworkinstanceIntraResourceleafRef = map[string]*ElementWithLeafRef{
	"/network-instance[name=]/mpls/static-mpls-entry[top-label=]/next-hop-group": {
	...
	"/network-instance[name=]/traffic-engineering/interface[interface-name=]": {
		REkvl: []ElementKeyValue{
			{"network-instance", "", ""},
			{"interface", "name", "string"},
		},
	},
	"/network-instance[name=]/traffic-engineering/interface[interface-name=]/admin-group": {
		REkvl: []ElementKeyValue{
			{"network-instance", "", ""},
			{"traffic-engineering", "", ""},
			{"admin-groups", "", ""},
	...
	"/network-instance[name=]/traffic-engineering/interface[interface-name=]/srlg-membership": {
		REkvl: []ElementKeyValue{
			{"network-instance", "", ""},
			{"traffic-engineering", "", ""},
...
var NetworkinstanceInterResourceleafRef = map[string]*ElementWithLeafRef{
	"/network-instance[name=]/interface[name=]": {
		REkvl: []ElementKeyValue{
			{"interface", "name", "string"},
			{"subinterface", "index", "int"},
		},
	},
	"/network-instance[name=]/vxlan-interface[name=]": {
		REkvl: []ElementKeyValue{
			{"tunnel-interface", "name", "string"},
			{"vxlan-interface", "index", "int"},
		},
	},
}

/*
var NetworkinstanceInterResourceleafRef = map[string][]ElementKeyValue{
	"/network-instance[name=]/interface[name=]": {
		{"interface", "name", "string"},
		{"subinterface", "index", "int"},
	},
...
// SrlNetworkinstanceReconciler reconciles a SrlNetworkinstance object
type SrlNetworkinstanceReconciler struct {
	client.Client
	Log    logr.Logger
...
type SrlNetworkinstanceTargetReconcileInfo struct {
	Target map[string]*SrlNetworkinstanceReconcileInfo
}

type SrlNetworkinstanceReconcileInfo struct {
	target              *string
	resource            *string
...
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlnetworkinstances,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlnetworkinstances/status,verbs=get;update;patch
...
// request for reconciliation of SrlNetworkinstance.
func (r *SrlNetworkinstanceReconciler) NetworkNodeMapFunc(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
	...
	os := &srlinuxv1alpha1.SrlNetworkinstanceList{}
...
func (r *SrlNetworkinstanceReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlNetworkinstanceReconciler) ValidateExternalLeafRefs(ctx context.Context, cm *string) (err error) {
	r.Log.Info("Validate External LeafRef Dependencies ...")

	d := make([][]byte, 0)
	d = append(d, []byte(*cm))

	for localLeafRef, leafRefInfo := range NetworkinstanceInterResourceleafRef {
	...
	r.Log.WithValues("NetworkinstanceIntraResourceleafRef", NetworkinstanceIntraResourceleafRef).Info("External leafref STATUS All")

	return nil
}

func (r *SrlNetworkinstanceReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstance) (err error) {
	r.Log.Info("Validate Local LeafRef Dependencies ...")

	// marshal data to json
	d := make([][]byte, 0)
	for _, obj := range *o.Spec.SrlNetworkinstance {
	...
	for localLeafRef, leafRefInfo := range NetworkinstanceIntraResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
		...
		// if not we dont have a leafref dependency in this resource
		remoteLeafRefPaths, localLeafRefPaths := r.FindLocalLeafRef(localLeafRef, d, ekvl, leafRefInfo.REkvl)
		r.Log.WithValues("Local LeafRef Path ", localLeafRef, "remoteLeafRefPaths", remoteLeafRefPaths, "localLeafRefPaths", localLeafRefPaths).Info("Internal Local/Remote LeafRef Paths")
		...
		for i, remoteLeafRefPath := range remoteLeafRefPaths {
			...
			rekvl := getHierarchicalElements(remoteLeafRefPath)
			rlvs := r.FindRemoteLeafRef(remoteLeafRefPath, d, rekvl)
			r.Log.WithValues("Remote LeafRef Path ", remoteLeafRefPath, "remote leafref values", rlvs).Info("Internal Remote LeafRef Values")
			found := false
			...
			for _, values := range rlvs {
				if values == rekvl[len(rekvl)-1].KeyValue {
					...
					r.Log.WithValues("localLeafRef", localLeafRef, "leafRefInfo", leafRefInfo).Info("Internal Remote Leafref FOUND, all good")
				}
			}
			if !found {
				leafRefInfo.LocalResolvedLeafRefInfo[localLeafRefPaths[i]] = &srlinuxv1alpha1.RemoteLeafRefInfo{
				...
				r.Log.WithValues("localLeafRef", localLeafRef, "leafRefInfo", leafRefInfo).Info("Internal Remote Leafref NOT FOUND, missing leaf reference")
			}
		}
		r.Log.WithValues("localLeafRef", localLeafRef, "leafRefInfo", leafRefInfo).Info("Internal leafref STATUS")
	}
	r.Log.WithValues("NetworkinstanceIntraResourceleafRef", NetworkinstanceIntraResourceleafRef).Info("Internal leafref STATUS All")
	return nil
}

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *SrlNetworkinstanceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("SrlNetworkinstance", req.NamespacedName)

	r.Log.WithValues("ObjectName", req.NamespacedName).Info("reconciling SrlNetworkinstance")
	...
	if err := r.Client.Get(ctx, req.NamespacedName, o); err != nil {
		...
		r.Log.WithValues(req.Name, req.Namespace).Error(err, "Failed to get SrlNetworkinstance config")
	...
	if o.DeletionTimestamp.IsZero() && !SrlNetworkinstancehasFinalizer(o) {
		r.Log.Info(
			...
			"newValue", srlinuxv1alpha1.SrlNetworkinstanceFinalizer,
		)
	...
	if o.DeletionTimestamp.IsZero() && SrlNetworkinstancehasFinalizer(o) {
		...
		o.Status.ConfigurationDependencyInternalLeafrefValidationDetails = make(map[string]*srlinuxv1alpha1.ValidationDetails2, 0)
		for localLeafRef, leafRefInfo := range NetworkinstanceIntraResourceleafRef {
			if len(leafRefInfo.LocalResolvedLeafRefInfo) > 0 {
				o.Status.ConfigurationDependencyInternalLeafrefValidationDetails[localLeafRef] = &srlinuxv1alpha1.ValidationDetails2{
	...
	level := int32(1)
	resource := "srlinux.henderiw.be" + "." + "SrlNetworkinstance" + "." + strcase.UpperCamelCase(o.Name)
	hkeys0 := make([]string, 0)
	for _, n := range *o.Spec.SrlNetworkinstance {
	...
	for _, target := range t {
		...
		if _, ok := o.Status.Target[target.TargetName]; !ok {
			o.Status.Target[target.TargetName] = &srlinuxv1alpha1.TargetStatus{
				...
				ErrorCount:   intPtr(0),
				ConfigurationDependencyParentValidationDetails:          make(map[string]*srlinuxv1alpha1.ValidationDetails2, 0),
				ConfigurationDependencyExternalLeafrefValidationDetails: make(map[string]*srlinuxv1alpha1.ValidationDetails2, 0),
			}
		}
		...
		r.Log.WithValues("Target", target.TargetName, "ParentDependencyFound", target.ParentDependencyFound).Info("Parent Dependency")
		if parentDependencyFound {
		...
		err = r.ValidateExternalLeafRefs(ctx, cm)
		if err != nil {
			return ctrl.Result{}, errors.Wrap(err, "failed to validate external leafRef")
		}

		validationSuccess := true
		o.Status.Target[target.TargetName].ConfigurationDependencyExternalLeafrefValidationDetails = make(map[string]*srlinuxv1alpha1.ValidationDetails2, 0)
		for localLeafRef, leafRefInfo := range NetworkinstanceInterResourceleafRef {
			if len(leafRefInfo.LocalResolvedLeafRefInfo) > 0 {
				o.Status.Target[target.TargetName].ConfigurationDependencyExternalLeafrefValidationDetails[localLeafRef] = &srlinuxv1alpha1.ValidationDetails2{
					LocalResolvedLeafRefInfo: make(map[string]*srlinuxv1alpha1.RemoteLeafRefInfo),
				}
				for localLeafRefPath, RemoteLeafRefInfo := range leafRefInfo.LocalResolvedLeafRefInfo {
					if *RemoteLeafRefInfo.DependencyCheck != srlinuxv1alpha1.DependencyCheckSuccess {
	...
	info := make(map[string]*SrlNetworkinstanceReconcileInfo)
	...
	for _, target := range t {
		...
		r.Log.Info("configuration status in reconcile",
			...
			"status", initialState)
		info[target.TargetName] = &SrlNetworkinstanceReconcileInfo{
			ctx:                 ctx,
		...
		stateMachine := newSrlNetworkinstanceStateMachine(o, r, &target.TargetName, info[target.TargetName])
	...
	for _, target := range t {
		...
		SrlNetworkinstancelogResult(info[target.TargetName], result[target.TargetName])
...
func (r *SrlNetworkinstanceReconciler) saveSrlNetworkinstanceStatus(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstance) error {
	...
	r.Log.Info("SrlNetworkinstance",
		"status", o.Status)

	if err := r.Client.Status().Update(ctx, o); err != nil {
		r.Log.WithValues(o.Name, o.Namespace).Error(err, "Failed to update SrlNetworkinstance ")
		return err
	}
	return nil
}
...
func (r *SrlNetworkinstanceReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlNetworkinstanceReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlNetworkinstanceReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
func (r *SrlNetworkinstanceReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstance) ([]string, []string, error) {
	...
	for localLeafRef, rekvl := range NetworkinstanceInterResourceleafRef {
		...
		// check if the leafref is configured in the resource
		// if not we dont have a leafref dependency in this resource
		lrd, lrp := r.FindLocalLeafRef(localLeafRef, d, ekvl, rekvl.REkvl)
		if len(lrd) != 0 {
...
func (r *SrlNetworkinstanceReconciler) FindSpecDiff(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstance) (bool, *[]string, error) {
...
func (r *SrlNetworkinstanceReconciler) FindTarget(ctx context.Context, o *srlinuxv1alpha1.SrlNetworkinstance) ([]*Target, bool, error) {
...
// SrlNetworkinstancehasFinalizer checks if object has finalizer
func SrlNetworkinstancehasFinalizer(o *srlinuxv1alpha1.SrlNetworkinstance) bool {
	return StringInList(o.Finalizers, srlinuxv1alpha1.SrlNetworkinstanceFinalizer)
}

func (info *SrlNetworkinstanceReconcileInfo) DeleteCache() error {
	if !info.o.DeletionTimestamp.IsZero() && SrlNetworkinstancehasFinalizer(info.o) {
...
// Update Cache
func (info *SrlNetworkinstanceReconcileInfo) UpdateCache(path string) error {

	// marshal data to json
	d := make([][]byte, 0)
	for _, obj := range *info.o.Spec.SrlNetworkinstance {
		o := make([]srlinuxv1alpha1.Networkinstance, 0)
		o = append(o, obj)
		dd := struct {
			NetworkInstance *[]srlinuxv1alpha1.Networkinstance `json:"network-instance"`
...
type SrlNetworkinstanceStateMachine struct {
	Object     *srlinuxv1alpha1.SrlNetworkinstance
	Reconciler *SrlNetworkinstanceReconciler
	Target     *string
	TargetName *string
...
// appendEvent
func (info *SrlNetworkinstanceReconcileInfo) appendEvent(reason, message string) {
	info.events = append(info.events, info.o.NewEvent(reason, message))
}

func newSrlNetworkinstanceStateMachine(o *srlinuxv1alpha1.SrlNetworkinstance,
	reconciler *SrlNetworkinstanceReconciler, n *string,
	info *SrlNetworkinstanceReconcileInfo) *SrlNetworkinstanceStateMachine {
	currentState := o.Status.Target[*n].ConfigStatus
	r := SrlNetworkinstanceStateMachine{
		Object:     o,
...
type SrlNetworkinstancestateHandler func(*SrlNetworkinstanceReconcileInfo) actionResult

func (o *SrlNetworkinstanceStateMachine) handlers() map[srlinuxv1alpha1.ConfigStatus]SrlNetworkinstancestateHandler {
...
func (o *SrlNetworkinstanceStateMachine) updateSrlNetworkinstanceStateFrom(initialState *srlinuxv1alpha1.ConfigStatus,
	info *SrlNetworkinstanceReconcileInfo) {
...
func (o *SrlNetworkinstanceStateMachine) ReconcileState(info *SrlNetworkinstanceReconcileInfo) actionResult {
	initialState := o.Object.Status.Target[*o.TargetName].ConfigStatus
	defer o.updateSrlNetworkinstanceStateFrom(initialState, info)
...
func (o *SrlNetworkinstanceStateMachine) handleNone(info *SrlNetworkinstanceReconcileInfo) actionResult {
...
func (o *SrlNetworkinstanceStateMachine) handleConfiguring(info *SrlNetworkinstanceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlNetworkinstanceStateMachine) handleDeleting(info *SrlNetworkinstanceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
