...
/*
	func (r *SrlInterfaceReconciler) validateLocalLeafRefs(o *srlinuxv1alpha1.Interface) (err error) {
		// marshal data to json
		dd := struct {
			Interface *srlinuxv1alpha1.Interface `json:"interface"`
		}{
		...
		for elementWithleafrefPath, elementWithleafref := range InterfaceIntraResourceleafRef {
...
func (r *SrlInterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if o.DeletionTimestamp.IsZero() && SrlInterfacehasFinalizer(o) {
		...
		/*
			validationSuccess := true
			for _, vo := range *o.Spec.SrlInterface {
...
/*
	func (r *SrlInterfaceReconciler) FindInterLeafRefDependencies(ctx context.Context, o *srlinuxv1alpha1.SrlInterface) ([]string, []string, error) {
		...
		leafRefDependencies := make([]string, 0)
		localLeafRefPaths := make([]string, 0)
		for localLeafRef, rekvl := range InterfaceInterResourceleafRef {
			// get the ekvl for the local leafref
			ekvl := getHierarchicalElements(localLeafRef)
