...
var TunnelinterfaceInternalResourceleafRef = map[string]*ElementWithLeafRef{}

var TunnelinterfaceExternalResourceleafRef = map[string]*ElementWithLeafRef{}

// SrlTunnelinterfaceReconciler reconciles a SrlTunnelinterface object
type SrlTunnelinterfaceReconciler struct {
	client.Client
	Log    logr.Logger
...
type SrlTunnelinterfaceTargetReconcileInfo struct {
	Target map[string]*SrlTunnelinterfaceReconcileInfo
}

type SrlTunnelinterfaceReconcileInfo struct {
	target              *string
	resource            *string
...
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srltunnelinterfaces,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srltunnelinterfaces/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srltunnelinterfaces/finalizers,verbs=update
// +kubebuilder:rbac:groups="",resources=secrets,verbs=get;list;watch;update
// +kubebuilder:rbac:groups="",resources=events,verbs=get;list;watch;create;update;patch
// +kubebuilder:rbac:groups="",resources=configmaps,verbs=get;list;watch;update

func (r *SrlTunnelinterfaceReconciler) publishEvent(request ctrl.Request, event corev1.Event) {
	reqLogger := r.Log.WithValues("SrlTunnelinterface", request.NamespacedName)
...
/*
	func (r *SrlTunnelinterfaceReconciler) updateEventHandler(e event.UpdateEvent) bool {
		_, oldOK := e.ObjectOld.(*srlinuxv1alpha1.SrlTunnelinterface)
		_, newOK := e.ObjectNew.(*srlinuxv1alpha1.SrlTunnelinterface)
		if !(oldOK && newOK) {
...
func (r *SrlTunnelinterfaceReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, option controller.Options) error {
	b := ctrl.NewControllerManagedBy(mgr).
		For(&srlinuxv1alpha1.SrlTunnelinterface{}).
...
// request for reconciliation of SrlTunnelinterface.
func (r *SrlTunnelinterfaceReconciler) NetworkNodeMapFunc(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
	...
	os := &srlinuxv1alpha1.SrlTunnelinterfaceList{}
...
func (r *SrlTunnelinterfaceReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlTunnelinterfaceReconciler) ValidateExternalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlTunnelinterface, cm *string) (err error) {
	r.Log.Info("Validate External LeafRef Dependencies ...")

	// marshal data to json
	d := make([][]byte, 0)
	for _, obj := range *o.Spec.SrlTunnelinterface {
		o := make([]srlinuxv1alpha1.Tunnelinterface, 0)
		o = append(o, obj)
		dd := struct {
			TunnelInterface *[]srlinuxv1alpha1.Tunnelinterface `json:"tunnel-interface"`
		}{
			TunnelInterface: &o,
		}
	...
	r.Log.WithValues("TunnelinterfaceExternalResourceleafRef", TunnelinterfaceExternalResourceleafRef).Info("External leafref STATUS All")

	return nil
}

func (r *SrlTunnelinterfaceReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlTunnelinterface) (err error) {
	...
	for localLeafRef, leafRefInfo := range TunnelinterfaceInternalResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
	...
	r.Log.WithValues("TunnelinterfaceInternalResourceleafRef", TunnelinterfaceInternalResourceleafRef).Info("leafref STATUS All")
	return nil
}

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *SrlTunnelinterfaceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("SrlTunnelinterface", req.NamespacedName)

	r.Log.WithValues("ObjectName", req.NamespacedName).Info("reconciling SrlTunnelinterface")
	...
	if err := r.Client.Get(ctx, req.NamespacedName, o); err != nil {
		...
		r.Log.WithValues(req.Name, req.Namespace).Error(err, "Failed to get SrlTunnelinterface config")
	...
	if o.DeletionTimestamp.IsZero() && !SrlTunnelinterfacehasFinalizer(o) {
		r.Log.Info(
			...
			"newValue", srlinuxv1alpha1.SrlTunnelinterfaceFinalizer,
		)
		o.Finalizers = append(o.Finalizers,
			srlinuxv1alpha1.SrlTunnelinterfaceFinalizer)
		err := r.Update(ctx, o)
		if err != nil {
	...
	if !o.DeletionTimestamp.IsZero() && SrlTunnelinterfacehasFinalizer(o) {
		if SrlTunnelinterfacehasOtherFinalizer(o) {
	...
	if err != nil {
		...
		case *TargetNotFoundError:
			...
			if !o.DeletionTimestamp.IsZero() && SrlTunnelinterfacehasFinalizer(o) {
				...
				o.Finalizers = removeString(o.Finalizers, srlinuxv1alpha1.SrlTunnelinterfaceFinalizer)
	...
	level := int32(1)
	resource := "srlinux.henderiw.be" + "." + "SrlTunnelinterface" + "." + strcase.UpperCamelCase(o.Name)
	hkeys0 := make([]string, 0)
	for _, n := range *o.Spec.SrlTunnelinterface {
	...
	for _, hkey0 := range hkeys0 {
		deletepaths = append(deletepaths, fmt.Sprintf("/tunnel-interface[name=%s]", hkey0))
	}
	...
	if o.DeletionTimestamp.IsZero() && SrlTunnelinterfacehasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlTunnelinterface"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
	...
	for _, target := range t {
		...
		r.Log.Info("configuration status in reconcile",
			"target", target.TargetName,
			"status", initialState)
		info[target.TargetName] = &SrlTunnelinterfaceReconcileInfo{
			ctx:                 ctx,
		...
		stateMachine := newSrlTunnelinterfaceStateMachine(o, r, &target.TargetName, info[target.TargetName])
	...
	for _, target := range t {
		...
		SrlTunnelinterfacelogResult(info[target.TargetName], result[target.TargetName])
...
func SrlTunnelinterfacelogResult(info *SrlTunnelinterfaceReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlTunnelinterface")
		//info.log.Info("stopping reconcile SrlTunnelinterface",
		//	"message", info.o.Status)
	}
}

func (r *SrlTunnelinterfaceReconciler) saveSrlTunnelinterfaceStatus(ctx context.Context, o *srlinuxv1alpha1.SrlTunnelinterface) error {
	t := metav1.Now()
	o.Status.DeepCopy()
	o.Status.LastUpdated = &t

	r.Log.Info("SrlTunnelinterface",
		"status", o.Status)

	if err := r.Client.Status().Update(ctx, o); err != nil {
		r.Log.WithValues(o.Name, o.Namespace).Error(err, "Failed to update SrlTunnelinterface ")
		return err
	}
	return nil
}

func (r *SrlTunnelinterfaceReconciler) getConfigMap(ctx context.Context, targetName *string) (*string, error) {
...
/*
	func (r *SrlTunnelinterfaceReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlTunnelinterface"] = string(d)
		cm.Data[fmt.Sprintf("SrlTunnelinterface.%s",*oName)] = string(d)
...
/*
	func (r *SrlTunnelinterfaceReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlTunnelinterface.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlTunnelinterface.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlTunnelinterfaceReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
func (r *SrlTunnelinterfaceReconciler) findPathInTree(x1 interface{}, ekvl []ElementKeyValue, idx int) bool {
...
func (r *SrlTunnelinterfaceReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlTunnelinterfaceReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlTunnelinterfaceReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
func (r *SrlTunnelinterfaceReconciler) FindSpecDiff(ctx context.Context, o *srlinuxv1alpha1.SrlTunnelinterface) (bool, *[]string, error) {
...
func (r *SrlTunnelinterfaceReconciler) FindTarget(ctx context.Context, o *srlinuxv1alpha1.SrlTunnelinterface) ([]*Target, bool, error) {
...
// SrlTunnelinterfacehasFinalizer checks if object has finalizer
func SrlTunnelinterfacehasFinalizer(o *srlinuxv1alpha1.SrlTunnelinterface) bool {
	return StringInList(o.Finalizers, srlinuxv1alpha1.SrlTunnelinterfaceFinalizer)
}

// SSrlTunnelinterfacehasOtherFinalizer checks if object has other finalizers
func SrlTunnelinterfacehasOtherFinalizer(o *srlinuxv1alpha1.SrlTunnelinterface) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlTunnelinterfaceFinalizer {
			return true
		}
	}
	return false
...
func (info *SrlTunnelinterfaceReconcileInfo) DeleteCache() error {
	if !info.o.DeletionTimestamp.IsZero() && SrlTunnelinterfacehasFinalizer(info.o) {
...
// Update Cache
func (info *SrlTunnelinterfaceReconcileInfo) UpdateCache(path string) error {

	// marshal data to json
	d := make([][]byte, 0)
	for _, obj := range *info.o.Spec.SrlTunnelinterface {
		o := make([]srlinuxv1alpha1.Tunnelinterface, 0)
		o = append(o, obj)
		dd := struct {
			TunnelInterface *[]srlinuxv1alpha1.Tunnelinterface `json:"tunnel-interface"`
...
type SrlTunnelinterfaceStateMachine struct {
	Object     *srlinuxv1alpha1.SrlTunnelinterface
	Reconciler *SrlTunnelinterfaceReconciler
	Target     *string
	TargetName *string
...
// appendEvent
func (info *SrlTunnelinterfaceReconcileInfo) appendEvent(reason, message string) {
	info.events = append(info.events, info.o.NewEvent(reason, message))
}

func newSrlTunnelinterfaceStateMachine(o *srlinuxv1alpha1.SrlTunnelinterface,
	reconciler *SrlTunnelinterfaceReconciler, n *string,
	info *SrlTunnelinterfaceReconcileInfo) *SrlTunnelinterfaceStateMachine {
	currentState := o.Status.Target[*n].ConfigStatus
	r := SrlTunnelinterfaceStateMachine{
		Object:     o,
...
type SrlTunnelinterfacestateHandler func(*SrlTunnelinterfaceReconcileInfo) actionResult

func (o *SrlTunnelinterfaceStateMachine) handlers() map[srlinuxv1alpha1.ConfigStatus]SrlTunnelinterfacestateHandler {
	return map[srlinuxv1alpha1.ConfigStatus]SrlTunnelinterfacestateHandler{
...
func (o *SrlTunnelinterfaceStateMachine) updateSrlTunnelinterfaceStateFrom(initialState *srlinuxv1alpha1.ConfigStatus,
	info *SrlTunnelinterfaceReconcileInfo) {
...
func (o *SrlTunnelinterfaceStateMachine) ReconcileState(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	initialState := o.Object.Status.Target[*o.TargetName].ConfigStatus
	defer o.updateSrlTunnelinterfaceStateFrom(initialState, info)

	if o.checkInitiateDelete() {
		// initiate cache delete
		info.log.Info("Initiating SrlTunnelinterfaceStateMachine deletion")
		info.DeleteCache()
	}
...
func (o *SrlTunnelinterfaceStateMachine) checkInitiateDelete() bool {
	if !o.Object.DeletionTimestamp.IsZero() && SrlTunnelinterfacehasFinalizer(o.Object) {
		// Delete requested
		switch *o.NextState {
		default:
			// new state deleting
...
func (o *SrlTunnelinterfaceStateMachine) handleNone(info *SrlTunnelinterfaceReconcileInfo) actionResult {
...
func (o *SrlTunnelinterfaceStateMachine) handleConfiguring(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlTunnelinterfaceStateMachine) handleConfigureSuccess(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlTunnelinterfaceStateMachine) handleConfigureFailed(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlTunnelinterfaceStateMachine) handleDeleting(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlTunnelinterfaceStateMachine) handleDeleteSuccess(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlTunnelinterfaceStateMachine) handleDeleteFailed(info *SrlTunnelinterfaceReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
