...
var SystemMtuInternalResourceleafRef = map[string]*ElementWithLeafRef{}

var SystemMtuExternalResourceleafRef = map[string]*ElementWithLeafRef{}

// SrlSystemMtuReconciler reconciles a SrlSystemMtu object
type SrlSystemMtuReconciler struct {
	client.Client
	Log    logr.Logger
...
type SrlSystemMtuTargetReconcileInfo struct {
	Target map[string]*SrlSystemMtuReconcileInfo
}

type SrlSystemMtuReconcileInfo struct {
	target              *string
	resource            *string
...
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlsystemmtus,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlsystemmtus/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=srlinux.henderiw.be,resources=srlsystemmtus/finalizers,verbs=update
// +kubebuilder:rbac:groups="",resources=secrets,verbs=get;list;watch;update
// +kubebuilder:rbac:groups="",resources=events,verbs=get;list;watch;create;update;patch
// +kubebuilder:rbac:groups="",resources=configmaps,verbs=get;list;watch;update

func (r *SrlSystemMtuReconciler) publishEvent(request ctrl.Request, event corev1.Event) {
	reqLogger := r.Log.WithValues("SrlSystemMtu", request.NamespacedName)
...
/*
	func (r *SrlSystemMtuReconciler) updateEventHandler(e event.UpdateEvent) bool {
		_, oldOK := e.ObjectOld.(*srlinuxv1alpha1.SrlSystemMtu)
		_, newOK := e.ObjectNew.(*srlinuxv1alpha1.SrlSystemMtu)
		if !(oldOK && newOK) {
...
func (r *SrlSystemMtuReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, option controller.Options) error {
	b := ctrl.NewControllerManagedBy(mgr).
		For(&srlinuxv1alpha1.SrlSystemMtu{}).
...
// request for reconciliation of SrlSystemMtu.
func (r *SrlSystemMtuReconciler) NetworkNodeMapFunc(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
	...
	os := &srlinuxv1alpha1.SrlSystemMtuList{}
...
func (r *SrlSystemMtuReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlSystemMtuReconciler) ValidateExternalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemMtu, cm *string) (err error) {
	r.Log.Info("Validate External LeafRef Dependencies ...")

	// marshal data to json
	dd := struct {
		Mtu *srlinuxv1alpha1.SystemMtu `json:"mtu"`
	}{
		Mtu: o.Spec.SrlSystemMtu,
	}
	d := make([][]byte, 0)
	...
	for localLeafRef, leafRefInfo := range SystemMtuExternalResourceleafRef {
	...
	r.Log.WithValues("SystemMtuExternalResourceleafRef", SystemMtuExternalResourceleafRef).Info("External leafref STATUS All")

	return nil
}

func (r *SrlSystemMtuReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemMtu) (err error) {
	...
	for localLeafRef, leafRefInfo := range SystemMtuInternalResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
	...
	r.Log.WithValues("SystemMtuInternalResourceleafRef", SystemMtuInternalResourceleafRef).Info("leafref STATUS All")
	return nil
}

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *SrlSystemMtuReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = r.Log.WithValues("SrlSystemMtu", req.NamespacedName)

	r.Log.WithValues("ObjectName", req.NamespacedName).Info("reconciling SrlSystemMtu")
	...
	if err := r.Client.Get(ctx, req.NamespacedName, o); err != nil {
		...
		r.Log.WithValues(req.Name, req.Namespace).Error(err, "Failed to get SrlSystemMtu config")
	...
	if o.DeletionTimestamp.IsZero() && !SrlSystemMtuhasFinalizer(o) {
		r.Log.Info(
			"adding finalizer",
			"existingFinalizers", o.Finalizers,
			"newValue", srlinuxv1alpha1.SrlSystemMtuFinalizer,
		)
		o.Finalizers = append(o.Finalizers,
			srlinuxv1alpha1.SrlSystemMtuFinalizer)
		err := r.Update(ctx, o)
		if err != nil {
	...
	if !o.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(o) {
		if SrlSystemMtuhasOtherFinalizer(o) {
	...
	if o.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(o) {
		...
		if err := r.saveSrlSystemMtuStatus(ctx, o); err != nil {
	...
	if err != nil {
		...
		case *TargetNotFoundError:
			...
			if !o.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(o) {
				...
				o.Finalizers = removeString(o.Finalizers, srlinuxv1alpha1.SrlSystemMtuFinalizer)
	...
	level := int32(2)
	resource := "srlinux.henderiw.be" + "." + "SrlSystemMtu" + "." + strcase.UpperCamelCase(o.Name)

	dependencies := make([]string, 0)

	deletepaths := make([]string, 0)
	deletepaths = append(deletepaths, "/system/mtu")
	...
	if o.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlSystemMtu"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
	...
	info := make(map[string]*SrlSystemMtuReconcileInfo)
	...
	for _, target := range t {
		...
		r.Log.Info("configuration status in reconcile",
			"target", target.TargetName,
			"status", initialState)
		info[target.TargetName] = &SrlSystemMtuReconcileInfo{
			ctx:                 ctx,
		...
		stateMachine := newSrlSystemMtuStateMachine(o, r, &target.TargetName, info[target.TargetName])
	...
	for _, target := range t {
		...
		SrlSystemMtulogResult(info[target.TargetName], result[target.TargetName])
...
func SrlSystemMtulogResult(info *SrlSystemMtuReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlSystemMtu")
		//info.log.Info("stopping reconcile SrlSystemMtu",
		//	"message", info.o.Status)
	}
}

func (r *SrlSystemMtuReconciler) saveSrlSystemMtuStatus(ctx context.Context, o *srlinuxv1alpha1.SrlSystemMtu) error {
	t := metav1.Now()
	o.Status.DeepCopy()
	o.Status.LastUpdated = &t

	r.Log.Info("SrlSystemMtu",
		"status", o.Status)

	if err := r.Client.Status().Update(ctx, o); err != nil {
		r.Log.WithValues(o.Name, o.Namespace).Error(err, "Failed to update SrlSystemMtu ")
		return err
	}
	return nil
}

func (r *SrlSystemMtuReconciler) getConfigMap(ctx context.Context, targetName *string) (*string, error) {
...
/*
	func (r *SrlSystemMtuReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlSystemMtu"] = string(d)
		cm.Data[fmt.Sprintf("SrlSystemMtu.%s",*oName)] = string(d)
...
/*
	func (r *SrlSystemMtuReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlSystemMtu.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlSystemMtu.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlSystemMtuReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
func (r *SrlSystemMtuReconciler) findPathInTree(x1 interface{}, ekvl []ElementKeyValue, idx int) bool {
...
func (r *SrlSystemMtuReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlSystemMtuReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlSystemMtuReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
func (r *SrlSystemMtuReconciler) FindSpecDiff(ctx context.Context, o *srlinuxv1alpha1.SrlSystemMtu) (bool, *[]string, error) {
...
func (r *SrlSystemMtuReconciler) FindTarget(ctx context.Context, o *srlinuxv1alpha1.SrlSystemMtu) ([]*Target, bool, error) {
...
// SrlSystemMtuhasFinalizer checks if object has finalizer
func SrlSystemMtuhasFinalizer(o *srlinuxv1alpha1.SrlSystemMtu) bool {
	return StringInList(o.Finalizers, srlinuxv1alpha1.SrlSystemMtuFinalizer)
}

// SSrlSystemMtuhasOtherFinalizer checks if object has other finalizers
func SrlSystemMtuhasOtherFinalizer(o *srlinuxv1alpha1.SrlSystemMtu) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlSystemMtuFinalizer {
			return true
		}
	}
	return false
...
func (info *SrlSystemMtuReconcileInfo) DeleteCache() error {
	if !info.o.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(info.o) {
...
// Update Cache
func (info *SrlSystemMtuReconcileInfo) UpdateCache(path string) error {

	// marshal data to json
	dd := struct {
		Mtu *srlinuxv1alpha1.SystemMtu `json:"mtu"`
	}{
		Mtu: info.o.Spec.SrlSystemMtu,
	}
	d := make([][]byte, 0)
	dj, err := json.Marshal(dd)
	if err != nil {
...
type SrlSystemMtuStateMachine struct {
	Object     *srlinuxv1alpha1.SrlSystemMtu
	Reconciler *SrlSystemMtuReconciler
	Target     *string
	TargetName *string
...
// appendEvent
func (info *SrlSystemMtuReconcileInfo) appendEvent(reason, message string) {
	info.events = append(info.events, info.o.NewEvent(reason, message))
}

func newSrlSystemMtuStateMachine(o *srlinuxv1alpha1.SrlSystemMtu,
	reconciler *SrlSystemMtuReconciler, n *string,
	info *SrlSystemMtuReconcileInfo) *SrlSystemMtuStateMachine {
	currentState := o.Status.Target[*n].ConfigStatus
	r := SrlSystemMtuStateMachine{
		Object:     o,
...
type SrlSystemMtustateHandler func(*SrlSystemMtuReconcileInfo) actionResult

func (o *SrlSystemMtuStateMachine) handlers() map[srlinuxv1alpha1.ConfigStatus]SrlSystemMtustateHandler {
	return map[srlinuxv1alpha1.ConfigStatus]SrlSystemMtustateHandler{
...
func (o *SrlSystemMtuStateMachine) updateSrlSystemMtuStateFrom(initialState *srlinuxv1alpha1.ConfigStatus,
	info *SrlSystemMtuReconcileInfo) {
...
func (o *SrlSystemMtuStateMachine) ReconcileState(info *SrlSystemMtuReconcileInfo) actionResult {
	initialState := o.Object.Status.Target[*o.TargetName].ConfigStatus
	defer o.updateSrlSystemMtuStateFrom(initialState, info)

	if o.checkInitiateDelete() {
		// initiate cache delete
		info.log.Info("Initiating SrlSystemMtuStateMachine deletion")
		info.DeleteCache()
	}
...
func (o *SrlSystemMtuStateMachine) checkInitiateDelete() bool {
	if !o.Object.DeletionTimestamp.IsZero() && SrlSystemMtuhasFinalizer(o.Object) {
		// Delete requested
		switch *o.NextState {
		default:
			// new state deleting
...
func (o *SrlSystemMtuStateMachine) handleNone(info *SrlSystemMtuReconcileInfo) actionResult {
...
func (o *SrlSystemMtuStateMachine) handleConfiguring(info *SrlSystemMtuReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemMtuStateMachine) handleConfigureSuccess(info *SrlSystemMtuReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemMtuStateMachine) handleConfigureFailed(info *SrlSystemMtuReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemMtuStateMachine) handleDeleting(info *SrlSystemMtuReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemMtuStateMachine) handleDeleteSuccess(info *SrlSystemMtuReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlSystemMtuStateMachine) handleDeleteFailed(info *SrlSystemMtuReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
