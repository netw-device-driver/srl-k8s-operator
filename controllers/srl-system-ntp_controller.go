...
var SystemNtpInternalResourceleafRef = map[string]*ElementWithLeafRef{}

var SystemNtpExternalResourceleafRef = map[string]*ElementWithLeafRef{
	"/ntp/network-instance": {
...
func (r *SrlSystemNtpReconciler) NetworkNodeMapFunc(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
...
func (r *SrlSystemNtpReconciler) ValidateParentDependency(ctx context.Context, cm *string, dependencies *[]string) (bool, error) {
	var x1 interface{}
...
func (r *SrlSystemNtpReconciler) ValidateExternalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemNtp, cm *string) (err error) {
	...
	for localLeafRef, leafRefInfo := range SystemNtpExternalResourceleafRef {
	...
	r.Log.WithValues("SystemNtpExternalResourceleafRef", SystemNtpExternalResourceleafRef).Info("External leafref STATUS All")

	return nil
}

func (r *SrlSystemNtpReconciler) ValidateLocalLeafRefs(ctx context.Context, o *srlinuxv1alpha1.SrlSystemNtp) (err error) {
	...
	for localLeafRef, leafRefInfo := range SystemNtpInternalResourceleafRef {
		// get the ekvl for the local leafref
		ekvl := getHierarchicalElements(localLeafRef)
	...
	r.Log.WithValues("SystemNtpInternalResourceleafRef", SystemNtpInternalResourceleafRef).Info("leafref STATUS All")
	return nil
}
...
func (r *SrlSystemNtpReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !o.DeletionTimestamp.IsZero() && SrlSystemNtphasFinalizer(o) {
		if SrlSystemNtphasOtherFinalizer(o) {
	...
	if o.DeletionTimestamp.IsZero() && SrlSystemNtphasFinalizer(o) {
		for _, target := range t {
			...
			if err := addSharedMemoryEntry(stringPtr(target.TargetName), stringPtr("SrlSystemNtp"), stringPtr(req.Name), stringSlicePtr(deletepaths)); err != nil {
				return ctrl.Result{}, err
			}
...
func SrlSystemNtplogResult(info *SrlSystemNtpReconcileInfo, result ctrl.Result) {
	if result.Requeue || result.RequeueAfter != 0 ||
		...
		info.log.Info("stopping reconcile SrlSystemNtp")
		//info.log.Info("stopping reconcile SrlSystemNtp",
...
func (r *SrlSystemNtpReconciler) getConfigMap(ctx context.Context, targetName *string) (*string, error) {
...
/*
	func (r *SrlSystemNtpReconciler) addEntryConfigMap(ctx context.Context, targetName, oName *string, deletepaths *[]string) error {
		dp := &DeletePaths{
			DeletePaths: deletepaths,
		}
		...
		//cm.Data["SrlSystemNtp"] = string(d)
		cm.Data[fmt.Sprintf("SrlSystemNtp.%s",*oName)] = string(d)
...
/*
	func (r *SrlSystemNtpReconciler) deleteEntryConfigMap(ctx context.Context, targetName, oName *string) error {
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
		}
		...
		if _, ok := cm.Data[fmt.Sprintf("SrlSystemNtp.%s",*oName)]; ok {
			delete(cm.Data, fmt.Sprintf("SrlSystemNtp.%s",*oName))
			if err := r.Update(ctx, cm); err != nil {
				r.Log.Error(err, "Failed to update configmap")
				return err
			}
...
/*
	func (r *SrlSystemNtpReconciler) GetRemoteleafRefResource(ctx context.Context, targetName *string, remoteleafRef *srlinuxv1alpha1.RemoteLeafRefInfo) (*string, error) {
		// get configmap
		cmKey := types.NamespacedName{
			Namespace: "nddriver-system",
			Name:      "nddriver-cm-" + *targetName,
...
func (r *SrlSystemNtpReconciler) findPathInTree(x1 interface{}, ekvl []ElementKeyValue, idx int) bool {
...
func (r *SrlSystemNtpReconciler) findLeafRefInTree(x1 interface{}, ekvl []ElementKeyValue, idx int, leafRefValues, localLeafRefPaths []string, lridx int) ([]string, []string) {
...
func (r *SrlSystemNtpReconciler) FindRemoteLeafRef(remoteLeafRef string, d [][]byte, rekvl []ElementKeyValue) []string {
...
func (r *SrlSystemNtpReconciler) FindLocalLeafRef(localLeafRef string, d [][]byte, ekvl, rekvl []ElementKeyValue) ([]string, []string) {
...
// SSrlSystemNtphasOtherFinalizer checks if object has other finalizers
func SrlSystemNtphasOtherFinalizer(o *srlinuxv1alpha1.SrlSystemNtp) bool {
	for _, f := range o.Finalizers {
		if f != srlinuxv1alpha1.SrlSystemNtpFinalizer {
			return true
		}
	}
	return false
...
func (info *SrlSystemNtpReconcileInfo) DeleteCache() error {
...
// Update Cache
func (info *SrlSystemNtpReconcileInfo) UpdateCache(path string) error {
...
func (o *SrlSystemNtpStateMachine) handleConfigureSuccess(info *SrlSystemNtpReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNtpStateMachine) handleConfigureFailed(info *SrlSystemNtpReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
...
func (o *SrlSystemNtpStateMachine) handleDeleteSuccess(info *SrlSystemNtpReconcileInfo) actionResult {
	return actionFinished{}
}

func (o *SrlSystemNtpStateMachine) handleDeleteFailed(info *SrlSystemNtpReconcileInfo) actionResult {
	cr, err := getCachStatus(o.Reconciler.Ctx, o.Target, info.resource, *info.level)
	if err != nil {
		return actionFailed{dirty: true, errorCount: *info.o.Status.Target[*o.TargetName].ErrorCount}
	}
